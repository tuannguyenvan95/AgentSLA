  ...
  return (
    <div className={`bg-slate-900/80 border ${statusInfo.border} rounded-2xl p-5 shadow-lg transition-all hover:shadow-cyan-500/10 flex flex-col justify-between relative overflow-hidden group`}>
      ...
      <div>
        ...
        {/* Role & Party Information Box */}
        <div className="mb-4 p-3 rounded-xl bg-slate-950/70 border border-slate-800/80">
          <div className="flex items-center justify-between text-xs mb-2.5 pb-2 border-b border-slate-800/70">
            <span className="text-slate-400 font-medium">Your Role:</span>
            {isCreator ? (
              <span className="px-2 py-0.5 rounded-md text-[11px] font-mono font-bold bg-cyan-950/90 text-cyan-300 border border-cyan-500/50 flex items-center gap-1 shadow-sm">
                <span>👑</span> Master Agent (Task Creator)
              </span>
            ) : isWorker ? (
              <span className="px-2 py-0.5 rounded-md text-[11px] font-mono font-bold bg-emerald-950/90 text-emerald-300 border border-emerald-500/50 flex items-center gap-1 shadow-sm">
                <span>⚡</span> Sub-Agent (Assigned Worker)
              </span>
            ) : (
              <span className="px-2 py-0.5 rounded-md text-[11px] font-mono text-slate-400 bg-slate-900 border border-slate-800 flex items-center gap-1">
                <span>👁️</span> Public Observer
              </span>
            )}
          </div>
          <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
            <div>
              <span className="text-slate-500 block">Master Agent (Creator):</span>
              <span className={isCreator ? 'text-cyan-300 font-bold' : 'text-slate-300'}>
                {formatAddress(job.creator)} {isCreator && '(You)'}
              </span>
            </div>
            <div className="text-right">
              <span className="text-slate-500 block">Sub-Agent (Worker):</span>
              <span className={isWorker ? 'text-emerald-300 font-bold' : 'text-slate-300'}>
                {job.worker && job.worker !== '0x0000000000000000000000000000000000000000'
                  ? `${formatAddress(job.worker)} ${isWorker ? '(You)' : ''}`
                  : 'Unclaimed (Open)'}
              </span>
            </div>
          </div>
      ...
      <div className="pt-3 border-t border-slate-800/80">
        ...
        {job.status === 0 && (
          <div>
            {isCreator ? (
              /* Creator cannot claim their own job - only cancel */
              <div className="flex items-center gap-2">
                <div className="flex-1 py-2 px-3 rounded-xl bg-slate-950/80 border border-slate-800 text-[11px] font-mono text-slate-400 flex items-center gap-1.5">
